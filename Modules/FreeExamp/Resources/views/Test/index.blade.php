...
<head>
    ...
    <title>PERMATAMALL | Test Online Gratis</title>
    <!-- Fonts-->
                    ...
                    <div class="panel-body" style="background: #3eb960;color: #fff;font-weight: bold;">
                        <div class="row">
                            <div class="header-profile-examp">
                                <div class="col-md-6 test-profile">{{ json_decode($Profile->keterangan)->kelas ?? '' }}</div>
                                <div class="col-md-6 test-profile">{{ json_decode($Profile->keterangan)->matpel ?? '' }}</div>
                            </div>
                        ...
                        <ul class='pagination text-center' id="pagination">
                            @foreach($Jumlahsoal as $key => $value)
                                <li id="{{ $value->id_soal_execute }}" data-id="{{ $key+1 }}" class="@if($value->jawaban_user != '') terjawab @else non_active @endif   @if($value->id_soal_execute == $soalSatu->id_soal_execute) active @endif">
                                    <a href="{{ route('FreeExamp.ETest.soal') }}?execute={{ $value->id_soal_execute }}" style="margin: 5px;border-radius: 4px;font-size: 16px;">{{ $key+1 }}</a>
                                </li>
                            @endforeach                                    
                        </ul>
...
<script>
    ...
    jQuery(document).ready(function() {
        jQuery("#target-content").load("{{ route('FreeExamp.ETest.soal') }}?execute={{ $soalSatu->id_soal_execute }}&number=1");
        jQuery("#pagination li").live('click', function(e) {
            ...
            jQuery("#target-content").load("{{ route('FreeExamp.ETest.soal') }}?execute="+pageNum+"&number="+number);
        });
    });
    ...
    function timer() {
      ...
      if (seconds == 0) {
        ...
        $.ajax({
            type: "GET",
            url: '{{ route("FreeExamp.ETest.finish") }}',
            data: {
            ...
            success: function(responses){
                ...
                swal({
                  ...
                  buttons:"Keluar",
                    ...
                    window.location.href = "{{ route('FreeExamp.finish',[encrypt($Profile->id_examp)]) }}";
                });
             }
        });
      } else {
    ...
    function functionAnswer (response){
        ...
        $.ajax({
            type: "POST",
            url: '{{ route("FreeExamp.ETest.answer") }}',
            data: {
                _token: "{{ csrf_token() }}",
                ans:ans,
                id:res,
            ...
            success: function(responses){
                ...
                if(typeof(element) != 'undefined' && element != null){
                    ...
                    jQuery("#target-content").load("{{ route('FreeExamp.ETest.soal') }}?execute="+responses+"&number="+number);
                }         
             }
        });
    }
