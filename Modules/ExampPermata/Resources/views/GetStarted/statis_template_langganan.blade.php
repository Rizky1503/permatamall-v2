...
<div class="container" style="padding-bottom: 80px; padding-top: 50px;" id="seach_page">          
  <div class="row"> 
      ...
      <div class="panel panel-default">
        ...
        <div class="panel-body" id="loading-dulu-two">
          <form method="post" enctype="multipart/form-data" id="form-all">
            ...
            <input type="hidden" name="_test_online" checked="form-control" value="" id="test_online">
            <div id="pending" style="display: none;">
                Lanjutkan Pembayaran
              </a>
            </div>

            <div class="form-group" id="pilih_tingkat" style="display: none;">
              <label for="email">Tingkat:</label>
              <select class="form-control select2" name="_jenis_tingkat" id="jenis_tingkat" style="width:100%" onchange="functiongetjenenislatihan(this.value)">
            ...
            <div class="form-group" id="pilih_tahun" style="display: none;">
              ...
              <select class="form-control select2" name="_tahun" id="tahun" style="width:100%" onchange="functiongetmatpel(this.value)">
                <option value="2020" selected="selected">Pilih Tahun</option>
            ...
            <div class="form-group" >
              <button type="button" class="btn btn-primary" id="mulai-test" style="float: right; display: none" onclick="confirmRequest()">Mulai Test</button>
            </div>
          </form>                   
        </div>
       </div>
    ...
    <div class="col-md-8">
      ...
      <div class="row" style="margin-top: 10px; display: none;" id="icon-samping-feature">
          ...
          <div class="panel panel-default">
            ...
            <div style="width: 33%; float: left;">
                <a style="cursor: pointer;" onclick="refresh()">
                  <img src="{!! asset('public/assets/images/icon/icon/back1.png') !!}" class="image-icon-examp">
...
function functiongetjenis(value){
    $.ajax({
        ...
        success: function(responses){
          if (responses.status == "in Progres") {
            if (responses.data_order.kondisi == 11 || responses.data_order.kondisi == 10 ) {
            ...
            $('#icon-samping-paket').hide();
            $('#icon-samping-feature').show();
            ...
            var option_tingkat = '<option value="" selected disabled>Pilih Tingkat </option>';
            $.each(responses.tingkat, function (key, value) {
              option_tingkat += '<option value="'+ value.tingkat + '">' + value.tingkat + '</option>';             
            })

            $('#ditampilkan').html(responses.data_order.keterangan + ' ' + responses.data_order.kondisi );
            $('#test_online').val(responses.data_order.id_order);
            $('#jenis_latihan').html(option);
            $('#jenis_tingkat').html(option_tingkat);
            $('#_kelas').val(responses.data_order.keterangan);
            $('#_kelas_siapun').val(responses.data_order.keterangan);
            $('#_tingkat_siapun').val(responses.data_order.kondisi);
            $('#_tingkat').val(responses.data_order.kondisi);
            $('#id_kelas').val(responses.id_kelas);
...
function functiongetjenenislatihan(value){
  $.ajax({
      type: "GET",
      url: '{{ route("ExampPermata.getjenislatihan") }}',
       data: {
            id:value,
            id_kelas : $('#id_kelas').val()
        },  
      success: function(responses){
        var option = '<option value="" selected disabled>Pilih Jenis Latihan </option>';
        $.each(responses.jenis, function (key, value) {
          option += '<option value="'+ value.jenis_paket + '">' + value.jenis_paket + '</option>';             
        })
        $('#jenis_latihan').html(option);
        $('#pilih_jenis_latihan').show();  
