  ...
  <div class="row">    
      <div class="col-md-12">
        <div class="permata-search" onclick="functionGetOverlay()" id="page_cari_halaman">
          <form action="{{ route('ExampPermata.video_langganan',encrypt($id)) }}" method="get">
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                  ...
                  <select class="form-control select2" name="matapelajaran" id="matapelajaran">
                      ...
                      @foreach($matpel as $key => $value)
                        <option value="{{$value->nama_matpel}}">{{$value->nama_matpel}}</option>
                      @endforeach
                  </select>
                </div>
              </div>            
  ...
  <div class="row">     
    <div class="col-md-12">
      <div class="row">
        ...
        <article class="col-md-2 video_post postType4" style="height: 290px; margin-bottom: 70px;">
          <div class="inner row m0" style="height: 330px;">
              <a href="{{ route('ExampPermata.nambah_view',encrypt([$ListVideo->slug,$id])) }}">
                <div class="row screencast m0">
                    <center>
                      <img src="{!! asset('public/images/bimbel_berbayar_icon/video.png') !!}" alt="" class="cast img-responsive" style="width: 74%">
                    </center>
              ...
              <div class="row m0 post_data">
                  <div class="row m0"><a href="{{ route('ExampPermata.nambah_view',encrypt([$ListVideo->slug,$id])) }}" class="post_title">{{ $ListVideo->title ?? '-' }}</a></div>
      ...
      <div style="float: right;">
        <nav aria-label="Page navigation example">
          <ul class="pagination">
            ...
            @else
            <li class="page-item"><a class="page-link" href="{{ route('ExampPermata.video_langganan',[encrypt($id),'page'=>$data->page-1]) }}">Sebelumnya</a></li>            
            @endif
            ...
            <li class="page-item disabled"><a class="page-link" href="">Selanjutnya</a></li>
            @else
            <li class="page-item"><a class="page-link" href="{{  route('ExampPermata.video_langganan',[encrypt($id),'page'=>$data->page+1]) }}">Selanjutnya</a></li>
            @endif            
          </ul>
        </nav>
      </div>
...
<script type="text/javascript">
  ...
  function getmatapelajaran(value){
    $.ajax({
      type: "GET",
      url: '{{ route("ExampPermata.matpel_video") }}',
      data: {
        tingkat : value,
      },
