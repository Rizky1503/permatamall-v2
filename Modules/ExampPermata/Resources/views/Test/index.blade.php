                    ...
                    <div class="panel-body" style="background: #3eb960;color: #fff;font-weight: bold;">
                        <div class="row">
                            <div class="header-profile-examp">
                                ...
                                <div class="col-md-4 test-profile">{{ json_decode($Profile->keterangan)->kelas ?? '' }}</div>
                                <div class="col-md-4 test-profile">{{ json_decode($Profile->keterangan)->matpel ?? '' }}</div>
                            </div>
                        ...
                        <ul class='pagination text-center' id="pagination">
                            @foreach($Jumlahsoal as $key => $value)
                                <li id="{{ $value->id_soal_execute }}" data-id="{{ $key+1 }}" class="@if($value->jawaban_user != '') terjawab @else non_active @endif   @if($value->id_soal_execute == $soalSatu->id_soal_execute) active @endif">
                                    <a href="{{ route('ExampPermata.ETest.soal') }}?execute={{ $value->id_soal_execute }}" style="margin: 5px;border-radius: 4px;font-size: 16px;">{{ $key+1 }}</a>
                                </li>
                            @endforeach                                    
                        </ul>
...
<script>
    ...
    jQuery(document).ready(function() {
        jQuery("#target-content").load("{{ route('ExampPermata.ETest.soal') }}?execute={{ $soalSatu->id_soal_execute }}&number=1");
        jQuery("#pagination li").live('click', function(e) {
            ...
            jQuery("#target-content").load("{{ route('ExampPermata.ETest.soal') }}?execute="+pageNum+"&number="+number);
        });
    });
    ...
    function timer() {
      ...
      if (seconds == 0) {
        ...
        $.ajax({
            type: "GET",
            url: '{{ route("ExampPermata.ETest.finish") }}',
            data: {
            ...
            success: function(responses){
                ...
                swal({
                  ...
                  buttons:"Keluar",
                    ...
                    window.location.href = "{{ route('ExampPermata.start') }}";
                });
             }
        });
      } else {
    ...
    function functionAnswer (response){
        ...
        $.ajax({
            type: "POST",
            url: '{{ route("ExampPermata.ETest.answer") }}',
            data: {
                _token: "{{ csrf_token() }}",
                ans:ans,
                id:res,
            ...
            success: function(responses){
                ...
                if(typeof(element) != 'undefined' && element != null){
                    ...
                    jQuery("#target-content").load("{{ route('ExampPermata.ETest.soal') }}?execute="+responses+"&number="+number);
                }         
             }
        });
    }
    ...
    function functionFinish(){
        var bag_8756321 = $('#8756321').html();
        if (bag_8756321 != 0) {
            swal({
              title: "Waktu anda masih tersedia",
