...
class ETestLanggananPermataController extends Controller
{
    /**
    ...
    {
        $Profile = json_decode(file_get_contents(ENV('APP_URL_API').'bo/berbayar/test/profile/'.decrypt(Session::get('token_examp_berbayar_'.Session::get('id_token_xmtrusr')))));

        if (empty($Profile)) {
            return redirect()->route('ExampPermataBimbelOnline.getStarted');
        }
        
        $Jumlahsoal   = json_decode(file_get_contents(ENV('APP_URL_API').'bo/berbayar/test/start/'.$Profile->id_examp.'/'.decrypt(Session::get('id_token_xmtrusr'))));

        return view('examppermata::Test.index_berbayar')->with([
            'Profile'       => $Profile,
    ...
    public function soal(Request $request)
    {
        ...
        $Jumlahsoal   = json_decode(file_get_contents(ENV('APP_URL_API').'bo/berbayar/test/soal/'.$execute)); 
        ...
        return "<div class='col-md-12'>    
                    <table style='width:100%'>
                      <tr>
                        ...
                        <td>
                          <img src='".ENV('APP_URL_API_RESOURCE').'images/Examp/5041/berbayar/Soal/'.$Jumlahsoal->soal."' style=''>
                        </td>
                      </tr>
                      <tr>
                        <td></td>
                      ...
                      <tr id='jawaban' style='display:none;'>
                        ...
                        <td>
                          <img src='".ENV('APP_URL_API_RESOURCE').'images/Examp/5041/berbayar/Pembahasan/'.$Jumlahsoal->pembahasan."' style=''>
                        </td>
                      </tr>
                    </table>
                </div>";
    ...
    public function answer(Request $request)
    {
      ...
      $response = $client->request('POST', ENV('APP_URL_API').'bo/berbayar/test/answer/soal', [
        'form_params'             => [
    ...
    {
      $client = new \GuzzleHttp\Client();
      $response = $client->request('POST', ENV('APP_URL_API').'bo/berbayar/test/finish', [
        'form_params'             => [
